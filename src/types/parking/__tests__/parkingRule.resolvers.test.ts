  ...
  describe("common for all", () => {
    ...
    it("ParkingRule.vehicles", async () => {
      ...
      function verifyResult(vehicles) {
        ...
        expect(vehicles[0].singleton).toBe(VehicleSelectorEnum.ALL);
        expect(vehicles[1].filter.toObject()).toMatchObject(vf1.toObject());
        expect(vehicles[2].filter.toObject()).toMatchObject(vf2.toObject());
        expect(vehicles[3].singleton).toBe(VehicleSelectorEnum.NONE);
      }
